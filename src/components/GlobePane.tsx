...
function GlobeScene() {
  ...
  const projectionVisualizationModel = useMemo(() => {
    if (!projectionVisualizationActive) {
      return null
    }
    ...
    <>
      ...
      <group ref={viewTiltGroupRef}>
        <group ref={viewSpinGroupRef}>
          ...
          {projectionVisualizationModel ? (
            <ProjectionSurface model={projectionVisualizationModel} />
          ) : null}
          ...
          <group ref={globeTiltGroupRef}>
            <group ref={globeSpinGroupRef}>
              <group ref={globeRollGroupRef}>
                ...
                >
                  <sphereGeometry args={[GLOBE_RADIUS, 96, 96]} />
                  {showDayNight || projectionVisualizationModel?.translucentGlobe ? (
                    <meshBasicMaterial
                      map={texture}
                      color="#ffffff"
                      transparent={Boolean(projectionVisualizationModel?.translucentGlobe)}
                      opacity={projectionVisualizationModel?.translucentGlobe ? 0.34 : 1}
                    />
                  ) : (
                    <meshStandardMaterial
                      map={texture}
